import {
  Component,
  OnInit,
  ChangeDetectionStrategy,
  OnDestroy,
  ChangeDetectorRef,
  Inject,
} from '@angular/core';
import { Subject } from 'rxjs';
import { ClientService } from '@optimo/core';
import { MAT_DIALOG_DATA, MatDialogRef } from '@angular/material/dialog';
import { FileDownloadHelper } from 'apps/dashboard/src/app/core/helpers/file-download/file-download.helper.ts';
import { HttpResponse } from '@angular/common/http';
import { takeUntil } from 'rxjs/operators';
import { NotificationsService } from 'apps/dashboard/src/app/core/services/notifications/notifications.service';
import { TranslateService } from '@ngx-translate/core';

@Component({
  selector: 'app-inventory-import-response-popup',
  templateUrl: './inventory-import-response-popup.component.html',
  styleUrls: ['./inventory-import-response-popup.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class InventoryImportResponsePopupComponent
  implements OnInit, OnDestroy {
  private unsubscribe$ = new Subject<void>();
  response: HttpResponse<Blob>;
  error: boolean;

  constructor(
    private client: ClientService,
    private cdr: ChangeDetectorRef,
    private dialogRef: MatDialogRef<InventoryImportResponsePopupComponent>,
    private notifier: NotificationsService,
    private fileDownloadHelper: FileDownloadHelper,
    @Inject(MAT_DIALOG_DATA) private data: FormData,
    private translate: TranslateService
  ) {}

  ngOnInit(): void {
    this.client
      .post('stockitems/import-excel', this.data, {
        responseType: 'blob',
        file: true,
      })
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        (response: HttpResponse<Blob>) => {
          this.response = response;
          this.showResultNotification();
          this.cdr.markForCheck();
        },
        () => {
          this.error = true;
          this.showResultNotification();
          this.cdr.markForCheck();
        }
      );
  }

  private showResultNotification(): void {
    if (this.error) {
      // 2 notifiers error
      this.notifier.sayError(
        this.translate.instant('INVENTORY.POPUPS.IMPORT_NOTIFICATION_ERROR')
      );
      this.dialogRef.close();
    }
    if (this.response && !this.hasError) {
      this.notifier.saySuccess(
        this.translate.instant('INVENTORY.POPUPS.IMPORT_NOTIFICATION_SUCCESS')
      );
      this.dialogRef.close();
    }
  }

  onExport(): void {
    this.fileDownloadHelper.downloadFromResponse(
      this.response,
      'application/ms-excel'
    );
  }

  get hasError(): boolean {
    return !!this.response.body.size;
  }

  onClose(): void {
    this.dialogRef.close();
  }

  ngOnDestroy(): void {
    this.unsubscribe$.next();
    this.unsubscribe$.complete();
  }
}
